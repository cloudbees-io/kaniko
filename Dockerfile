...
# Corrected go install with proper module path (no /v2 suffix)
...
# Build executor and warmer with caching enabled
RUN \
...
# Use musl busybox since it's statically compiled
FROM busybox:musl AS busybox

FROM scratch AS kaniko-base-slim

# Create /kaniko with world write permission for non-root runs
