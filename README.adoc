...
| String
| No
| The component_id of the component that the artifact belongs to.
If specified, the component must belong to either the same org as the current workflow run or its sub-orgs.
If not specified, defaults to the current workflow run’s component ${{cloudbees.component.id}}.
|===
